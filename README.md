...
### 使用GPU卡训练模型
如果要以并行的分布式方式训练模型，则需要协调训练过程(术语“模型副本”指在数据子集上训练的模型副本)

简单地采用模型参数异步更新方法会导致训练性能无法达到最佳，因为单个模型副本在训练时使用的可能是过时的模型参数，反之，如果采用完全同步的更新后的参数，其速度堪比最慢的模型副本。

在具有多个GPU卡的工作站，每个GPU的速度大致相当，具有足够的内存来运行整个模型，我们选择按照以下方式设计训练系统：

* 在每个GPU上放一个模型副本
* 等待所有GPU完成一批数据的处理工作，然后同步更新模型参数。

模型示意图如下 

![avatar](https://www.tensorflow.org/images/Parallelism.png)

每个GPU都会针对一批唯一的数据计算推理和梯度，这种设置可以有效地将一大批数据划分到各个GPU上。

这种设置要求所有GPU都共享模型参数，众所周知，将数据传输到GPU或从中向外传输数据的速度非常慢。因此，我们决定在CPU上存储和更新所有模型参数，当所有GPU均处理完一批新数据时，系统会将一组全新的模型参数传输给相应的GPU

GPU会同步运行，GPU的所有梯度将累积并求平均值，模型参数会更新为所有模型副本的梯度平均值

实例，比如batch_size设置为120在单GPU上运行，2个GPU运行的时候，则批次大小设置为64.
